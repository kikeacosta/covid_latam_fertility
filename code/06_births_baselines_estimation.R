...
# first round to exclude outliers
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
test <- 
  dt %>% 
  mutate(bts = bts + 1) %>% 
...
test2step <- function(ge = "Choco"){
  ...
  # test_2step1 %>% 
  #   ggplot()+
  #   geom_point(aes(date, bts), col = "black")+
  #   geom_line(aes(date, bts), col = "black")+
  #   geom_ribbon(aes(date, ymin = bsn_lc, ymax = bsn_uc), fill = "red", alpha = 0.5)+
  #   geom_ribbon(aes(date, ymin = bsn_lp1, ymax = bsn_up1), fill = "red", alpha = 0.4)+
  #   geom_ribbon(aes(date, ymin = bsn_lp3, ymax = bsn_up3), fill = "red", alpha = 0.3)+
  #   geom_ribbon(aes(date, ymin = bsn_lp5, ymax = bsn_up5), fill = "red", alpha = 0.2)+
  #   # geom_ribbon(aes(date, ymin = bsn_lp, ymax = bsn_up), fill = "red", alpha = 0.2)+
  #   geom_line(aes(date, bsn, linetype = edu), col = "red")+
  #   geom_vline(xintercept = c(ymd("2019-12-31")), 
  #              linetype = "dashed")+
  #   scale_x_date(breaks = seq(ymd('2010-01-01'),ymd('2022-01-01'), by = '1 year'),
  ...
  #   labs(title = "first-step fitting")+
  #   theme_bw()
  
  test_2step2 <- 
    test_2step1 %>% 
    # mutate(w = ifelse(w == 0 | bts < bsn_lp3 | bts > bsn_up3, 0, 1)) %>% 
